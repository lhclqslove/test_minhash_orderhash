...
minhash::ReadFilter::~ReadFilter() {}

void minhash::MinHashReadFilter::initialize(ReadData &rD) {
    this->rD = &rD;
...
void minhash::MinHashReadFilter::generateRandomNumbers(size_t n) {
    if (randNumbers)
        delete[] randNumbers;
...
void minhash::MinHashReadFilter::getFilteredReads(kMer_t sketch[],
...
void minhash::MinHashReadFilter::getFilteredReads(const std::string &s,
                                         std::vector<read_t> &results) {
    results.clear();
...
kMer_t minhash::MinHashReadFilter::kMerToInt(const std::string &s) {
    size_t l = s.length();
    kMer_t result = 0;
...
// in speed
char minhash::MinHashReadFilter::baseToInt(const char base) {
    return (base & 0b10) | ((base & 0b100) >> 2);
}

void minhash::MinHashReadFilter::string2Sketch(const std::string &s, kMer_t *sketch, std::vector<kMer_t> &kMers, std::vector<kMer_t> &hashes) {
...
void minhash::MinHashReadFilter::hashKMer(kMer_t kMer, std::vector<kMer_t> &hashes) {
    for (size_t l = 0; l < n; l++)
        hashes[l] = hasher(kMer^randNumbers[l]);
}

void minhash::MinHashReadFilter::string2KMers(const std::string &s, const size_t k,
...
void minhash::MinHashReadFilter::populateHashTables(const std::vector<kMer_t> &sketches) {
